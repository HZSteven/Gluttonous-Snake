...
struct Snake {
    ...
    int y;
    int dir;
}snake[484]; // queue Without Head And Tail

void clemap(void) { // clean The Map
    Map[ 0] = "######################";
    Map[ 1] = "#                    #";
    Map[ 2] = "#                    #";
    Map[ 3] = "#                    #";
...
void clesna(void) { // clean the snake
	for(int i = 1; i <= 483; ++i) {
		snake[i].x = snake[i].y = 0;
	}
}

void clecor(void) { // clean the corpse
	for(int i = length; i <= 483; ++i) {
		snake[i].x = snake[i].y = 0;
	}
}
...
inline int startPlaying() {
	while(1) { // gameLoop
        ...
        if(flag1 == 1) { // ifAnyKeyDown
        	...
        	//newDirectionUpd
            if     ((newinp == 68 || newinp == 'd') && direction != 3) {
            	direction = 1;
			}
            else if((newinp == 83 || newinp == 's') && direction != 4) {
            	direction=2;
			}
            else if((newinp == 65 || newinp ==  'a') && direction != 1) {
            	direction = 3;
			}
            else if((newinp == 87 || newinp == 'w') && direction != 2) {
            	direction = 4;
			}
			else if(newinp == 32){
				//controlPageOutput
                ...
                while(1) {
                    system("cls");
                    cout << "-Pause-" << endl;
                    string c[6] = {"Speed:", "Delaytime:", "Wall:", "Ghost:", "Corpse:", "Exit"};
                    for(int i = 0; i < 6; i++){
                        if(con - 1 == i) {
                        	cout << "&";
						}
